...
cmd_start() {
    ...
    # Run in background, redirect output and stdin, and capture PID
    nohup bash "$script_path" --loop --instance-id "$instance_id" < /dev/null > "/tmp/agent-${agent_type}-${instance_id}.log" 2>&1 &
    local pid=$!
    
    # Wait a moment to check if process started successfully
    sleep 1
    if ! kill -0 "$pid" 2>/dev/null; then
        echo -e "${RED}Error: Agent process failed to start${RESET}"
        # Check log for errors
        if [ -f "/tmp/agent-${agent_type}-${instance_id}.log" ]; then
            echo -e "${YELLOW}Last log entries:${RESET}"
            tail -5 "/tmp/agent-${agent_type}-${instance_id}.log" | sed 's/^/  /'
        fi
        exit 1
    fi
    ...
    sqlite3 "$DB_FILE" "
        ...
        VALUES ('$agent_type', '$instance_id', $pid, 'idle', datetime('now'), 'Started');
    " 2>/dev/null || {
    ...
    echo -e "  Log: /tmp/agent-${agent_type}-${instance_id}.log"
}
