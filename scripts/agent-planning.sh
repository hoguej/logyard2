...
cleanup() {
    log_info "Shutting down planning agent (instance: $INSTANCE_ID)..."
...
EOF
    ...
    log_info "Invoking headless Cursor agent for planning..."
    log_info "Prompt file: $prompt_file"

    if ! run_cursor_agent "$PROJECT_ROOT" "$prompt_file"; then
        log_error "Cursor agent failed for planning task $task_id"
    ...
    if [ ! -f "$plan_doc" ] && [ "${AGENT_ALLOW_PLACEHOLDER_OUTPUT:-0}" = "1" ]; then
        cat > "$plan_doc" <<EOF
# Planning: $rtitle
...
EOF
    ...
    if [ ! -f "$plan_doc" ]; then
        log_error "Planning document not found: $plan_doc"
        update_task_status "$task_id" "failed" "" "Planning document not created"
        return 1
    fi
...
if [ "$LOOP_MODE" = true ]; then
    ...
    while true; do
        ...
        # Try to claim a task
        task_id=$(claim_task "planning" "$AGENT_NAME" || true)
